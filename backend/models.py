...
class User(Base):
    ...
    workoutlogs: Mapped[list[WorkoutLog]] = relationship("WorkoutLog", back_populates="user", cascade="all, delete")

    @property
    def profile_pic_path(self) -> str:
        if self.profile_pic:
            return f"/home/yassine/Projects/GymTracker/backend/media/profiles/{self.profile_pic}"
        return f"/home/yassine/Projects/GymTracker/backend/media/profiles/default.png"
...
class WorkoutLog(Base):
    ...
    user: Mapped[User] = relationship("User", back_populates="workoutlogs")